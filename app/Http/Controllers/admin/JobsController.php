...
class JobsController extends Controller
{
    public function index()
    {
        ...
        $getJobDetails = Job::selectRaw('job_id,job_title,address_1,address_2,job_status_id,working_employee_id,job_status_id,start_date,end_date')->where('is_active', 1)->where('is_deleted', 0)->orderBy('created_at','DESC')->get();
    ...
    {
        $getJobDetails = Job::selectRaw('job_id,job_title,address_1,address_2,job_status_id,working_employee_id,start_date,end_date')->where('is_active', 0)->where('is_deleted', 0)->orderBy('updated_at','DESC')->get();
    ...
    if (sizeof($getJobNotes) > 0)
    {
        foreach($getJobNotes as $single_note)
        {
            ...
            <div class="col-xs-2">
                <a data-toggle="tooltip" data-placement="top" class="view-note-images" title="Remove" data-id ="'. $single_note->job_id .'">
                    <i class="ti-image"></i>
                </a>
            </div>
...
public function showFilterwiseJob(Request $request) {
   ...
   $getJobDetails = DB::select("SELECT jb.job_title,jb.super_name,jb.working_employee_id,jb.start_date,jb.end_date,jb.company_clients_id,jb.job_id,jb.job_status_id,cmp.name FROM jobs AS jb JOIN companies AS cmp ON cmp.company_id = jb.company_id WHERE jb.is_deleted = 0  {$jobStatusCond} ORDER BY jb.created_at DESC");
